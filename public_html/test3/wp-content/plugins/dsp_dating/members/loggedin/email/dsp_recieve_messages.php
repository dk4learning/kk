...
<div class="box-border">
    <div class="box-pedding">
       <?php if($count_messages > 0):?> 
        <div class="heading-submenu"><strong><?php echo __('My Messages', 'wpdating'); ?></strong></div>
...
//+++++++++++++++++++++++++++++++++++++++++++++++++++++++
                ...
                if ($check_couples_mode->setting_status == 'Y') {
                    $strQuery = "SELECT *,MAX(m.sent_date) AS receive_date FROM $dsp_user_emails_table m,$dsp_user_profiles p WHERE m.sender_id = p.user_id AND m.receiver_id=$user_id and m.delete_message=0 group by m.sender_id";
                } else {
                    $strQuery = "SELECT *,MAX(m.sent_date) AS receive_date FROM $dsp_user_emails_table m,$dsp_user_profiles p WHERE m.sender_id = p.user_id AND m.receiver_id=$user_id and m.delete_message=0 AND p.gender!='C' group by m.sender_id";
                }
                ...
                if ($bolIfSearchCriteria) {
                    $strQuery = $strQuery . " ORDER BY receive_date desc ";
...
// ------------------------------------------------End Paging code------------------------------------------------------ // 
            ...
            //     echo $message->sender_id."<br>";
            //     echo $message->receive_date."<br>";
            // }
            // die();
                foreach ($my_messages as $message) {
                    $display_sender_name = $wpdb->get_row("SELECT * FROM $dsp_user_table WHERE ID = '$message->sender_id'");
                    $set_date=$wpdb->get_row("SELECT sent_date FROM $dsp_user_emails_table WHERE receiver_id=$user_id AND sender_id = '$message->sender_id' order by message_id desc LIMIT 1");

                    $message_date = date("$dateFormat $timeFormat", strtotime($set_date->sent_date));
...
// -------------------------------- END OF PRINT PAGING LINKS ------------------------------------- //
            ...
            <div class="btn-delete">
                ...
                <input type="button" class="dsp_submit_button  dspdp-btn dspdp-btn-sm dspdp-btn-warning" name="delmsg" value="<?php echo __('Delete Selected', 'wpdating') ?>" onclick="delete_dsp_emails();"/>
            </div>
        </form>
       <?php else: ?>
