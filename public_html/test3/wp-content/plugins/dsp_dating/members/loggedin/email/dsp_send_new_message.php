						...
						<div class="dsp-xs-8 dspdp-col-sm-8">
							...
							<?php } else if ( $check_user_favourites > 0 ) {
								...
								<select name="friend_id" class="field1 dsp-xs-8 dsp-form-control dspdp-form-control">
									...
									$myfriends_list = $wpdb->get_results( "SELECT * FROM (SELECT  user_id, favourite_user_id   FROM $dsp_user_favourites_table   where user_id ='$user_id'  UNION  SELECT  friend_uid FROM $dsp_my_friends_table fren where user_id ='$user_id') as tjoin " );
