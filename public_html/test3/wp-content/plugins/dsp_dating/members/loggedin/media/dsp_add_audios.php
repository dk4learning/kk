        ...
        if ($sizeOK && $typeOK) {
            ...
            switch ($_FILES['file-upload']['error']) {
                    ...
                    if ($success) {
                        ...
                        if ($count_uploaded_audios < $check_audio_count->setting_value) {  // check condition Number of Audios in A Profiles.
                            if ($check_approve_audios_status->setting_status == 'Y') {  // if Audio approve status is Y then Audio Automatically Approved.
                                ...
                                $result[] = " $audio_file &nbsp;" . __('Uploaded successfully.', 'wpdating');
                                dsp_add_news_feed($user_id, 'audio', $insertid);
                                ...
                                $result[] = __('Your audio files will be approved within 24 hours.', 'wpdating');
                            ...
                            $limit_msg = __('You can only upload <#COUNT#> Audios', 'wpdating');
                        ...
                        $error_uploading = __('Error uploading <#FILE_NAME#> Please try again.', 'wpdating');
                ...
                default:
                    ...
                    $error_uploading = __('System error encountered while uploading <#FILE_NAME#>', 'wpdating');
            ...
            $result[] = __('No file selected.', 'wpdating');
        } else {

            $error_uploading = __('<#FILE_NAME#> cannot be uploaded.', 'wpdating');
...
if ($Action == "Del" && !empty($get_audio_Id)) {
    ...
    if ($audio_name != "") {
        ...
        dsp_delete_news_feed(get_current_user_id(), 'audio', $get_audio_Id);
    } // END if($audio_name!="")
...
<div class="box-border">
    <div class="box-pedding dsp-form-container">
        <div class="heading-submenu"><strong><?php echo __('Add Audio:', 'wpdating'); ?></strong></div>
        <div align="left">
                ...
                ?>
                        ...
                        <li class="dspdp-col-sm-6 dsp-sm-6 dspdp-col-xs-12 dsp-xs-12">
                            ...
                            <div style=" text-align:center;" align="center">
                                <a class="dspdp-btn dspdp-btn-sm dspdp-btn-danger dsp-btn-danger dsp-btn-sm" href="<?php echo $root_link . "media/add_audio/Action/Del/audio_Id/" . $audio_file_id . "/"; ?>" onclick="if (!confirm('<?php echo __('Are you sure you want to delete this Audio file?', 'wpdating'); ?>'))
                                        return false;"><span style="cursor:pointer;"><?php echo __('Delete', 'wpdating'); ?></span></a></div>
                            </div>
                        </li>
                <?php } ?>
...
// -------------------------------- END OF PRINT PAGING LINKS ------------------------------------- //
        ...
        <div class="dsp-box-container dsp-space">
            <form  class="dspdp-box-border" name="frmaddaudio" action="<?php echo $page_name ?>" method="post" enctype="multipart/form-data">
                <div >
                    <div >
                       <span><?php echo __('Add Audio:', 'wpdating'); ?>&nbsp;</span>
                       ...
                       <div class="dspdp-form-group"> <span classs="dspdp-checkbox ">
                            <span  classs="dspdp-vertical-middle"><?php echo __('Private:', 'wpdating');?></span>
                    ...
                    <div class="btn-row">
                        <input type="submit" name="submit" class="dsp_submit_button dspdp-btn dspdp-btn-default" value="<?php echo __('Add Audio', 'wpdating') ?>">
                    </div>
                </div>
            </form>
        </div>
