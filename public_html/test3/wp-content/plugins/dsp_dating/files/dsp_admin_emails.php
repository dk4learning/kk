...
$total_results1 = $wpdb->get_var("SELECT COUNT(*) as Num FROM $dsp_user_emails_table WHERE fake_key = 1 Order by sent_date desc");
}
else{
...
$next = $page + 1;
if(is_plugin_active('admin_message_reply/admin_message_reply.php')){
    $total_results1 = $total_results1+1;
}
...
<form class="dsp-form" name="adminmailsfrm" method="post">
    <table cellpadding="0" cellspacing="0" border="0" class="widefat">
        <tr>
            ...
            <?php if(is_plugin_active('admin_message_reply/admin_message_reply.php')){?>
            ...
            <?php } ?>
            <th scope="col"><?php echo __('Delete', 'wpdating') ?></th>
        </tr>
        <?php
        ...
        if (isset($_POST['search'])) {
            ...
            foreach ($search_username as $username) {
                $user_id = $username->ID;
                if(is_plugin_active('admin_message_reply/admin_message_reply.php')) {
                    $admin_mails_list = $wpdb->get_results("SELECT * FROM $dsp_user_emails_table where sender_id='$user_id' AND fake_key=1 Order by sent_date desc LIMIT $start, $limit");
                }
                else{
                ...
                foreach ($admin_mails_list as $email) {
                    ...
                    <?php if(is_plugin_active('admin_message_reply/admin_message_reply.php')){?>
                        <td><?php echo $subject?></div></td>
                        <td class = "short" id="<?php echo $message_id?>"><?php echo substr($text_message, 0, 60) ?> <button type="button" onclick="showmessage('<?php echo sanitize_text_field($text_message) ?>')" id="btnReply" data-toggle="modal" data-target="#replyModal"> Read more.. </button></div></td>
                    <?php }
                    else{?>
                    <td>
                    ...
                    <?php if(is_plugin_active('admin_message_reply/admin_message_reply.php')){
                            do_action('show_message_reply_message',$message_id, sanitize_text_field($text_message));
                    } ?>
            ...
            if(is_plugin_active('admin_message_reply/admin_message_reply.php')) {
                $admin_mails_list = $wpdb->get_results("SELECT * FROM $dsp_user_emails_table where fake_key=1 Order by sent_date desc LIMIT $start, $limit");
            }
            else{
            ...
            foreach ($admin_mails_list as $email) {
                ...
                if ($chk_receiver_exist != 0 && $chk_sender_exist != 0) {
                    ...
                    <?php if(is_plugin_active('admin_message_reply/admin_message_reply.php')){?>
                        <td><?php echo $subject?></div></td>
                        <td class = "short" id="<?php echo $message_id?>"><?php echo substr($text_message, 0, 60) ?> <button type="button" onclick="showmessage('<?php echo sanitize_text_field($text_message) ?>','<?php echo $message_id?> ')" id="btnReply" data-toggle="modal" data-target="#replyModal"> Read more.. </button></div></td>
                    <?php } ?>
                    <?php if(is_plugin_active('admin_message_reply/admin_message_reply.php')){?>
                    <?php }
                    else{?>
...
// -------------------------------- END OF PRINT PAGING LINKS ------------------------------------- //
                ...
                <div class="btn-delete-right">
                    ...
                    <input class="button" type="submit" name="submit" value="<?php echo __('Delete', 'wpdating') ?>" /></div>
            </td>
        </tr>
    </table>
</form>
