    ...
    <div class="user-info-home">
        <div class="top-slider-info dspdp-featured">
            <div class="join-info search_box dspdp-search-box">
                <?php /* ?><div class="heading-bar"><b class="changebox" id="searchbox"><?php echo __('Quick search', 'wpdating');?></b> <?php if(!is_user_logged_in()){?><b class="reg_popoup" id="freebox" title=""><?php echo __('Join Free', 'wpdating');?></b><?php }?></div><?php */ ?>

                <div class="join-box  join-searchbox dspdp-row">
                    <div class="dspdp-col-sm-2 dsp-sm-2 dspdp-h2 dsp-h2 dspdp-bold dsp-bold"><?php echo __('JOIN IT IS FREE!', 'wpdating'); ?><!-- <img class="dspdp-search-title-img dspdp-img-responsive" src="<?php echo plugins_url('dsp_dating/templates/template2/images/join-free.jpg'); ?>"  /> --></div>
                                    ...
                                    dsp(document).ready(function() {
                                       ...
                                       /* dsp('#register').click(function() {
                                            ...
                                            if (jQuery.trim(username).length == 0) {
                                                alert_text += "<?php echo __('Please Enter Username.', 'wpdating'); ?>\n";
                                             }
                                            var email = dsp('input[name="email"]').val();
                                            if (jQuery.trim(email).length == 0) {
                                                    alert_text += "<?php echo __('Please Enter Email address.', 'wpdating'); ?>\n";
                                            }
                                            var confirm_email = dsp('input[name="confirm_email"]').val();
                                            if (jQuery.trim(email).length == 0) {
                                                    alert_text += "<?php echo __('Please Confirm email address.', 'wpdating'); ?>\n";
                                            }
                                            <?php if ($check_terms_page_mode->setting_status == 'Y') { ?>
                                                    if (!dsp('input[name="terms"]').is(':checked')) {
                                                        alert_text += "<?php echo __('Please Check terms and conditions.', 'wpdating'); ?>\n";
                                                   }
                                            <?php } ?>
                                            if (alert_text != "") {
                                                alert(alert_text);
