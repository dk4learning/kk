    ...
    if (isset($_REQUEST['action'])) {
        if ($_REQUEST['action'] == 'send_request') {
            ...
            if ($check_request == 0) {
                ...
                $update = mysqli_query("update $dsp_chat_request SET request_status=0, time='" . date('g:i A') . "', date='" . date('Y-m-d') . "' where sender_id='$user_id' and receiver_id='$member_id'");
            }
        }


        if ($_REQUEST['action'] == 'accept_request') {
            $update = mysqli_query("update $dsp_chat_request SET request_status=1, time='" . date('g:i A') . "', date='" . date('Y-m-d') . "' where sender_id='$member_id' and receiver_id='$user_id'");
        }
    }
}
                                                ...
                                                if ($check_free_mode->setting_status == "N") {  // free mode is off 
                                                    ...
                                                    if ($check_free_trail_mode->setting_status == "N") {
                                                        ...
                                                        if ($check_membership_msg == "Expired") {
                                                            ...
                                                            ?>
                                                            <p><?php echo __('You must be a premium member to use the chat feature.', 'wpdating'); ?> 
                                                                <a href="dsp_upgrade.html"  class="textlink MenuiPhone"><?php echo __('Upgrade Here.', 'wpdating'); ?></a>
                                                            </p>
                                                            <?php
