...
if ( isset( $_POST['delete-thread'] ) ){
    if ( isset( $_POST['delete-thread-id'] ) && !empty( $_POST['delete-thread-id'] ) ){
        ...
        $status = $wpdb->query("UPDATE {$dsp_user_emails_table} SET delete_message = 1 WHERE thread_id IN($thread_ids)");
        if ( $status > 0){ ?>
            <script>
...
$sql_query = "SELECT COUNT(*) FROM (SELECT user_email.thread_id FROM {$dsp_user_emails_table} user_email
                    WHERE user_email.sender_id IN ({$included_sender_sql_query}) AND receiver_id = {$current_user->ID} AND user_email.delete_message = 0 
...
<div class="box-border">
    <div class="box-pedding">
       <?php if( $total_count > 0 ){ ?>
        ...
        <form name="frmdelmessages" action="" method="post" class="message-block-inbox">
            <div class="gray-title-head">
                ...
                $sql_query = "SELECT min(user_email.sender_id) sender_id, user_email.thread_id, max(user_email.sent_date) max_receive_date,
                    ...
                    (SELECT picture FROM {$dsp_members_photos_table} WHERE user_id = min(user_email.sender_id) LIMIT 1) image_name,
                    ...
                    (SELECT COUNT(*) FROM {$dsp_user_emails_table} WHERE thread_id = user_email.thread_id
                        AND sender_id = min(user_email.sender_id) AND receiver_id = {$current_user->ID}
                        AND message_read = 'N' AND delete_message = 0) AS unread_count
                    FROM {$dsp_user_emails_table} user_email
                    WHERE user_email.sender_id IN ({$included_sender_sql_query}) AND user_email.receiver_id = {$current_user->ID} AND user_email.delete_message = 0 
                ...
                foreach ($messages as $message) {
                    ...
                    <div class="dsp_vertical_scrollbar">
                        <ul class="no-list email-page <?php echo ( $message->unread_count > 0 ) ? 'dspdp-bg-info' : ''; ?>">
                            ...
                            <?php if ( $message->unread_count > 0 ) {
                                ?>
                                <a href="<?php echo "$message_url/view_message/?from=inbox&thread={$message->thread_id}&act=read"; ?>">
                                    <?php echo $message->subject; ?>&nbsp
                                    <span>(<?php echo $message->unread_count; ?>)</span>
                                </a>
                            <?php } else { ?>
                                <a href="<?php echo "{$message_url}/view_message/?from=inbox&thread={$message->thread_id}"; ?>">
