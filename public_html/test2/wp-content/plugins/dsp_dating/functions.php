		...
		if ($count_member_images > 0) {
			...
			if ($member_exist_picture->picture == "") {
				...
				if ($check_gender->gender == 'M') {
					...
					$Mem_Image_path = WPDATE_URL . "/images/members.jpg";
				}
			} else {
	...
	function check_membership($access_feature_name, $user_id) { 
		...
		if ($premium_access_features > 0) {
			...
			if ($check_member_payment > 0 && $isUnderPremiumPlan) {
                ...
                if (
                    ( ! isset( $name1 ) || ( isset( $name1 ) && empty( $name1 ) ) ) &&
                    (
                        $creditMode->setting_status == 'N' ||
                        ( $no_of_credits >= $giftSettingValue && $creditMode->setting_status == 'Y' )  ||
                        $check_member_payment > 0
                    )
                )  {
					$msg = "Access";
				} else {
...
if (!function_exists('check_force_profile_feature')) {
	...
	function check_force_profile_feature($access_feature_name, $user_id) {
		...
		$user_profile = $wpdb->get_row("SELECT status_id FROM $dsp_user_profiles WHERE user_id=$user_id");
		$status_id = isset($user_profile->status_id) ? $user_profile->status_id: '';
		$dsp_memberships_table = $wpdb->prefix . DSP_MEMBERSHIPS_TABLE;
		$dsp_payments_table = $wpdb->prefix . DSP_PAYMENTS_TABLE;
...
if (!function_exists('dsp_get_member_image_urls')){
    /** Get thumb size image url of the member.
     *
     * @param int $user_id
     ...
     * @return stdClass
     */
    function dsp_get_member_image_urls($user_id, $image_name, $private, $is_fav, $gender)
    {
        $site_url       = get_site_url();
        $user_image_obj = new stdClass();

        if ($private == 'Y' && !$is_fav) {
            $user_image_obj->image_orig = $user_image_obj->image_250 = $user_image_obj->image_350 = "{$site_url}/wp-content/plugins/dsp_dating/images/private-photo-pic.jpg";
            return $user_image_obj;
        }

        if (!empty($image_name)) {
            if (file_exists( WP_CONTENT_DIR . "/uploads/dsp_media/user_photos/user_{$user_id}/thumbs/thumb_{$image_name}")){
                $user_image_obj->image_orig = get_site_url() . "{$site_url}/wp-content/uploads/dsp_media/user_photos/user_{$user_id}/{$image_name}";
                $user_image_obj->image_250  = get_site_url() . "/wp-content/uploads/dsp_media/user_photos/user_{$user_id}/thumbs1/thumb_{$image_name}";
                $user_image_obj->image_350  = get_site_url() . "/wp-content/uploads/dsp_media/user_photos/user_{$user_id}/thumbs/thumb_{$image_name}";
                return $user_image_obj;
            }
        }
        ...
        switch ($gender) {
            case 'F':
                $user_image_obj->image_orig = $user_image_obj->image_250 = $user_image_obj->image_350 = "{$site_url}/wp-content/plugins/dsp_dating/images/female-generic.jpg";
                return $user_image_obj;

            case 'C':
                $user_image_obj->image_orig = $user_image_obj->image_250 = $user_image_obj->image_350 = "{$site_url}/wp-content/plugins/dsp_dating/images/couples-generic.jpg";
                return $user_image_obj;

            default:
                $user_image_obj->image_orig = $user_image_obj->image_250 = $user_image_obj->image_350 = "{$site_url}/wp-content/plugins/dsp_dating/images/male-generic.jpg";
                return $user_image_obj;
        }
    }
}
