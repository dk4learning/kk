        ...
        if ($user_id != 0 && $news_feed_user != "") {
            ...
            if ($notification_row != null) {
                ...
                if ($notification_row->type == 'send_email')
                    $notification_text = $displayed_member_name . ' ' . __('just sent you a Message.', 'wpdating');
                else if ($notification_row->type == 'add_favourites')
                    $notification_text = $displayed_member_name . ' ' . __('just added you as a favorite.', 'wpdating');
                else if ($notification_row->type == 'friend_request')
                    $notification_text = $displayed_member_name . ' ' . __('just sent you a friend request.', 'wpdating');
                else if ($notification_row->type == 'view_profile')
                    $notification_text = $displayed_member_name . ' ' . __('just viewed your profile.', 'wpdating');
                else if ($notification_row->type == 'send_wink')
                    $notification_text = $displayed_member_name . ' ' . __('just sent you a wink.', 'wpdating');
                else if ($notification_row->type == "status")
                    $notification_text = $displayed_member_name . ' ' . __('just added new status update.', 'wpdating');
                else if ($notification_row->type == "login")
                    $notification_text = $displayed_member_name . ' ' . __('just logged in.', 'wpdating');
                else if ($notification_row->type == "logout")
                    $notification_text = $displayed_member_name . ' ' . __('just logged off.', 'wpdating');
                else if ($notification_row->type == "video")
                    $notification_text = $displayed_member_name . ' ' . __('just added new video.', 'wpdating');
                else if ($notification_row->type == "audio")
                    $notification_text = $displayed_member_name . ' ' . __('just added new audio.', 'wpdating');
                else if ($notification_row->type == "gallery_photo")
                    $notification_text = $displayed_member_name . ' ' . __('just added new photo.', 'wpdating');
                else if ($notification_row->type == "profile_photo")
                    $notification_text = $displayed_member_name . ' ' . __('just added new profile photo.', 'wpdating');
