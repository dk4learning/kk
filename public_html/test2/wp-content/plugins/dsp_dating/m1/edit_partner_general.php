...
echo __('Partner Profile', 'wpdating') . ' ' . __('General', 'wpdating');
;
?></h1>
     ...
     <?php
        ...
        if ($check_approve_profile_status->setting_status == 'Y') {
            if (isset($profile_updated) && $profile_updated == true) {
                ?>
                <div class="success-message"><?php echo __('Your profile has been updated!', 'wpdating') ?></div>
                <?php
            }
        } else {
        ...
        <li><input type="hidden" name="mode"  value="update"/>
            ...
            <input type="button" onclick="editPartnerGeneral('true', '<?php echo __('Fields should not be empty.', 'wpdating') ?>')" name="submit1" value="<?php _e(__('Update', 'wpdating')); ?>" />
