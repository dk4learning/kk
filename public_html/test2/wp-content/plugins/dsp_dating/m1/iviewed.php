...
if ($check_couples_mode->setting_status == 'Y') {
    $strQuery = "SELECT p.user_id FROM $dsp_user_profiles_table p, $dsp_counter_hits_table h where h.member_id=p.user_id and p.status_id=1 and h.user_id=$user_id  GROUP BY h.member_id";
} else {
    $strQuery = "SELECT p.user_id FROM $dsp_user_profiles_table p, $dsp_counter_hits_table h where h.member_id=p.user_id and p.status_id=1 AND p.gender!='C' AND h.user_id=$user_id  GROUP BY h.member_id";
}

$strQuery = $strQuery . " ORDER BY p.user_profile_id desc LIMIT 20";
...
if (count($search_members) == 0) {
    ...
    <div style="text-align: center;">
        <?php echo __('No record found.', 'wpdating'); ?>
    </div>

    <?php
} else {
    ?>
    <div class="swipe_div" id="mainIViewed" style="height: 106px">
        <ul id="swipe_ulIViewed"  style="padding-left:0px;text-align: left; top: 0px; bottom: auto; left: 0px; margin: 0px; width: 2169px; height: 82px; float: none; position: absolute; right: auto; z-index: auto;list-style:none;">
            <?php
