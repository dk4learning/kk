...
$dsp_emails_draft_table                = $wpdb->prefix . DSP_MESSAGES_DRAFT_TABLE;
...
if ($version != $check_plugin_version) {
    if ($check_plugin_version == '4.8.1' || $check_plugin_version == '4.8' || $version == '4.8.2' || $version == '4.8.3' || $version == '4.8.3.1' || $version == '4.8.4' || $version == '4.8.4.1' || $version == '5.0' || $version == '5.1' || $version == '5.1.1' || $version == '5.1.2' || $version == '5.1.3' || $version == '5.1.3.1' || $version == '5.1.4' || $version == '5.2' || $version == '5.3' || $version == '5.4' || $version == '5.5' || $version == '5.6' || $version == '5.7' || $version == '5.8' || $version == '5.9' || $version == '5.9.1' || $version == '5.10' || $version == '6.0.0' || $version == '6.1' || $version == '6.2' || $version == '6.3' || $version == '6.3.1' || $version == '6.4' || $version == '6.4.1' || $version == '6.4.2' || $version == '6.4.3' || $version == '6.4.4' || $version == '6.4.4.1' || $version == '6.4.5' || $version == '6.5' || $version == '6.5.1' || $version == '6.5.2' || $version == '6.5.3' || $version == '6.5.4' || $version == '6.5.5' || $version == '6.5.6' || $version == '6.5.7' || $version == '6.5.8' || $version == '6.5.9' || $version == '6.6.0' || $version == '6.6.1' || $version == '6.6.2' || $version == '6.6.3' || $version == '6.6.4' || $version == '6.6.5' || $version == '6.6.6' || $version == '6.6.7' || $version == '6.6.8' || $version == '6.6.9' || $version == '6.7.0' || $version == '6.7.1'  || $version == '6.7.2' || $version == '6.8.0' || $version == '6.8.1' || $version == '6.8.2' || $version == '7.0.0' || $version == '7.0.1' || $version == '7.0.2' || $version == '7.0.3' || $version == '7.1.0' ) {
        ...
        $setupValues = array(
            ...
            'DSP_MAKE_PRO_PIC_PRIVATE'                               => 'Make your profile picture Private',
            ...
            'DSP_STATUS_UPDATE_NEW'                                  => 'Current Status',
            'DSP_NEW_RECORDS_NOT_FOUND'                              => 'Sorry, we could not find anyone near you. Start a new search.',
            'DSP_NEW_PLEASE_CHECK_YOUR_EMAIL_FOR_LOGIN_DETAIL'       => 'Alright! Your username and password has been emailed to you. Please check your email.',
            'DSP_NEW_EMPTY'                                          => 'No saved searches yet.',
            'DSP_NEW_PROFILE_MAKE_PRIVATE'                           => 'Make these profile questions private',
            'DSP_GIVE_SEARCH_TITLE'                                  => 'Give title to your saved search',
            'DSP_MIN_AGE'                                            => 'Minimum age',
            'DSP_MAX_AGE'                                            => 'Maximum age',
            'DSP_MIN_SEARCH_AGE'                                     => 'Minimum search age',
            'DSP_MAX_SEARCH_AGE'                                     => 'Maximum search age',
            'DSP_STEALTH_MODE_DESCRIPTION'                           => 'Select ON to enable offline mode on your site',
            'DSP_SETTINGS_HEADER_CREDITS_PLAN'                       => 'Credits Plan'
        );
        ...
        if ($wpdb->get_var("show tables like '$dsp_template_images_table'") != $dsp_template_images_table) {
    ...
    //  Create Table DSP_MESSAGES_DRAFT_TABLE
    if ( $wpdb->get_var("show tables like '$dsp_emails_draft_table'" ) != $dsp_emails_draft_table ) {
        $wpdb->query("CREATE TABLE IF NOT EXISTS `$dsp_emails_draft_table` (
    ...
    if ( $wpdb->get_var( "show tables like '$DSP_CREDITS_PLAN_TABLE'" ) != $DSP_CREDITS_PLAN_TABLE ) {
        $wpdb->query("CREATE TABLE $DSP_CREDITS_PLAN_TABLE (
            `credits_plan_id` INT NOT NULL AUTO_INCREMENT PRIMARY KEY,
            `plan_name` VARCHAR(80) NOT NULL,
            `amount` VARCHAR(60) NOT NULL,
            `no_of_credits` INT(70) NOT NULL) 
    ...
    //Alter table enum in search_criteria

    $wpdb->query("ALTER TABLE $dsp_user_search_criteria_table CHANGE `with_pictures` `with_pictures` ENUM('Y','N','P') CHARACTER SET utf8 COLLATE utf8_unicode_ci NULL DEFAULT NULL");

    //add column in payment table

    $sql_query = "SELECT * FROM information_schema.COLUMNS WHERE 
                        TABLE_SCHEMA = '$dsp_temp_payments_table' AND COLUMN_NAME = 'app_start_date'";
    if (!$wpdb->query($sql_query)){
        $sql_query = "ALTER TABLE $dsp_temp_payments_table 
	    	    ADD COLUMN `app_start_date` DATETIME,
                ADD COLUMN `app_expiration_date` DATETIME,
                ADD COLUMN `app_payment_date` DATETIME,
                ADD COLUMN `purchase_from` VARCHAR(255)";
        $wpdb->query($sql_query);
    }

    $sql_query = "SELECT * FROM information_schema.COLUMNS WHERE 
                        TABLE_SCHEMA = '$dsp_temp_payments_table' AND COLUMN_NAME = 'recurring_profile_status'";
    if (!$wpdb->query($sql_query)){
        $sql_query = "ALTER TABLE $dsp_temp_payments_table 
	    	    ADD COLUMN `recurring_profile_status` boolean DEFAULT 0;";
        $wpdb->query($sql_query);
    }

    $sql_query = "SELECT * FROM information_schema.COLUMNS WHERE 
                        TABLE_SCHEMA = '$dsp_payments_table' AND COLUMN_NAME = 'app_start_date'";
    if (!$wpdb->query($sql_query)){
        $sql_query = "ALTER TABLE $dsp_payments_table 
	    	    ADD COLUMN `app_start_date` DATETIME,
                ADD COLUMN `app_expiration_date` DATETIME,
                ADD COLUMN `app_payment_date` DATETIME,
                ADD COLUMN `purchase_from` VARCHAR(255)";
    ...
    $sql_query = "SELECT * FROM information_schema.COLUMNS WHERE 
                        TABLE_SCHEMA = '$dsp_payments_table' AND COLUMN_NAME = 'recurring_profile_status'";
    if (!$wpdb->query($sql_query)){
        $sql_query = "ALTER TABLE $dsp_payments_table 
	    	    ADD COLUMN `recurring_profile_status` boolean DEFAULT 0;";
        $wpdb->query($sql_query);
    ...
    /**
     * create dsp_tmp_members_partner_photos_table, if not exists
     */
    $dsp_tmp_members_partner_photos_table = $wpdb->prefix . DSP_TMP_MEMBERS_PARTNER_PHOTOS_TABLE;
    $wpdb->query( "CREATE TABLE $dsp_tmp_members_partner_photos_table (t_photo_id INT NOT NULL AUTO_INCREMENT PRIMARY KEY,t_user_id  INT NOT NULL,t_picture VARCHAR(250) NOT NULL,t_status_id INT NOT NULL) CHARACTER SET `utf8` COLLATE `utf8_unicode_ci`" );

    $dsp_gender_list = $wpdb->prefix . DSP_GENDER_LIST_TABLE;
    $old_gender_exist = $wpdb->get_var("SELECT COUNT(*) FROM $dsp_gender_list WHERE 
            gender = 'DSP_MAN' OR gender = 'DSP_WOMAN'  or gender = 'DSP_COUPLE' ");
    if ($old_gender_exist > 0){
        $wpdb->query("DELETE FROM $dsp_gender_list WHERE gender = 'DSP_MAN' OR gender = 'DSP_WOMAN'  or gender = 'DSP_COUPLE'");
        $wpdb->query( "INSERT INTO `$dsp_gender_list` 
                            (`id`, `gender`, `enum`, `editable`) 
                            VALUES
                            (1, 'Man', 'M', 'N'),
                            (2, 'Woman', 'F', 'N'),
                            (3, 'Couple', 'C', 'N')
    ...
    $sql_query = "SELECT * FROM information_schema.COLUMNS WHERE 
                        TABLE_NAME = '{$dsp_zipcode_table}' AND COLUMN_NAME = 'f1'";
    if ($wpdb->query($sql_query)){
        $sql_query = "ALTER TABLE {$dsp_zipcode_table} 
                            DROP COLUMN `f1`, 
                            DROP COLUMN `city`, 
                            DROP COLUMN `f2`, 
                            DROP COLUMN `state`, 
