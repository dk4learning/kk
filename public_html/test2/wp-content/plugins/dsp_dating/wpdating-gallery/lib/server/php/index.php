...
if (isset($_REQUEST['albumId']) and $_REQUEST['albumId'] != '') {
...
class CustomUploadHandler extends UploadHandler
{
    ...
    {
        parent::validate($uploaded_file, $file, $error, $index, $content_range);
        if ($this->count_uploaded_images()) {
            return true;
        }

        $limit_msg         = __('You can Upload only <#COUNT#> photos.', 'wpdating');
    ...
    protected function handle_file_upload(
        ...
        if ($this->check_photo_approve_status()) {
            ...
            $file->message = __('Uploaded successfully.', 'wpdating');
            if (function_exists('dsp_add_news_feed')) {
                dsp_add_news_feed($this->user_id, 'gallery_photo', $wpdb->insert_id);
            }
            ...
            $file->message = __('Pictures you uploaded will be approved within 24 hours.', 'wpdating');
    ...
    public function delete(
        ...
        foreach ($response as $name => $deleted) {
            if ($deleted) {
                $row = $wpdb->get_row("SELECT * FROM $this->dsp_galleries_photos WHERE image_name = '$name' AND user_id = '$this->user_id' AND album_id = '$this->album_id'");
                ...
                dsp_delete_news_feed((int)get_current_user_id(),'gallery_photo',(int)$row->gal_photo_id);
            }
        }
    ...
    public function count_uploaded_images()
    {
        ...
        if(!empty($dsp_album_id)){
            ...
            $count_uploaded_images = $wpdb->get_var("SELECT COUNT(*) as Num FROM $this->dsp_galleries_photos where album_id = 0  AND user_id = $this->user_id AND status_id=1");
        if ($count_uploaded_images < $check_image_count->setting_value) {
            return true;
            }
        }
