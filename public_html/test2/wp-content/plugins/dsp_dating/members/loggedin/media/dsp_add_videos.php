                    ...
                    if ($success) {
                        ...
                        if (isset($count_uploaded_video) && $count_uploaded_video < $check_video_count->setting_value) {  // check condition Number of Audios in A Profiles.
                            if ($check_approve_videos_status->setting_status == 'Y') {  // if Audio approve status is Y then Audio Automatically Approved.
                                ...
                                $result[] = " $video_file &nbsp;" . __('Uploaded successfully.', 'wpdating');



                                dsp_add_news_feed($user_id, 'video', $insertid);
                                ...
                                $result[] = __('Videos you uploaded will be approved within 24 hours.', 'wpdating');
                            ...
                            $limit_msg = __('You can only upload <#COUNT#> videos ', 'wpdating');
...
if ($Action == "Del" && !empty($get_video_Id)) {
    ...
    if ($video_name != "") {
        ...
        dsp_delete_news_feed(get_current_user_id(), 'video', $get_video_Id);

    } // END if($audio_name!="")

    $delete_video_msg =  __('Deleted Video.', 'wpdating');
...
<div class="box-border">
    <div class="box-pedding">
        <div class="heading-submenu"><strong><?php echo __('Add Videos', 'wpdating'); ?></strong></div>
            ...
            <form name="frmaddvideo" class="dspdp-box-border" enctype="multipart/form-data" action="<?php echo $page_name; ?>" method="post">
                ...
                <div>
                    ...
                    <div>
                        ...
                        <div class="dspdp-form-group dsp-form-group"><span><?php echo __('Add Video:', 'wpdating'); ?>&nbsp;</span>

                        <span><input class="dspdp-form-control dsp-form-control" type="file" name="file-upload" value=""></span></div>



                        <div class="dspdp-form-group"><span><?php echo __('Private:', 'wpdating'); ?><input name="private" type="checkbox" value="Y"/> </span></div>

                        <span><input type="hidden" name="txtmode" id="txtmode" value="add"></span>

                    </div>

                    <div class="btn-row"><input type="submit" name="submit" class="dsp_submit_button  dspdp-btn dspdp-btn-default" value="<?php echo __('Upload', 'wpdating') ?>"></div>
