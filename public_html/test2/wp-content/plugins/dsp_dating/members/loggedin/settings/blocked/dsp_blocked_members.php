...
<div class="box-border">
    <div class="box-pedding">
        <div class="heading-submenu"><strong><?php echo __('BLOCKED', 'wpdating'); ?></strong></div></br></br>
        <div class="dsp-row">
            <?php
            if ($total_results1 > 0) {
                ?>
                <form name="delblockedmembersfrm" action="" method="post">
                    <div class="title-text dsp-sm-12 margin-btm-1 dsp-h4"><strong><?php echo __('Blocked Members', 'wpdating') ?></strong></div>
                    <?php
...
//$blocked_members = $wpdb->get_results("SELECT * FROM $dsp_blocked_members_table WHERE user_id = '$user_id' ORDER BY blocked_id");
                    ...
                    foreach ($blocked_members as $Member) {
                        ...
                        <div class="dsp-sm-3 dsp-text-center"><div class="image-container dsp-block-member-item">
                            ...
                            if ($check_couples_mode->setting_status == 'Y') {
                                ...
                                <?php } ?>
                            <?php } ?>
                            <span style="float:left; width:100%;" onclick="delete_blocked_member(<?php echo $blocked_id ?>);" class="dsp-btn-default dsp-delete_blocked_member"><?php echo __('Unblock', 'wpdating') ?></span> 
                        </div></div>
                        <?php
                        $i++;
                        unset($favt_mem);
            ...
            <?php } else { ?>
                <div style="text-align:center"><strong><?php echo __('No Blocked Members!', 'wpdating') ?></strong></div>
                    <?php } ?>
        </div>
    </div>
</div>