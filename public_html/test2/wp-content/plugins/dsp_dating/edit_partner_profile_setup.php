...
//Check to make sure that the Country field is not empty
    if (trim($_POST['cmbCountry']) == '0') {
        $nameError = __('You forgot to select your country.', 'wpdating');
        $hasError = true;
    } else {
    ...
    if (trim($_POST['txtaboutme']) === '') {
        $aboutmeError = __('You forgot to enter About Me.', 'wpdating');
        $hasError = true;
    } else {
        $aboutme = $aboutme;
    }
    ...
    if (!isset($hasError)) {
        if ($mode == "update") {
            if ($check_approve_profile_status->setting_status == 'Y') {  // if Profile approve status is Y then Profile Automatically Approved.
                ...
                $profile_approval_message = __('Your profile will be updated within 24 hours.', 'wpdating');
...
// ************************* INSERT PROFILE QUESTION OPTION INFORMATION *****************************//
            ...
            foreach ($question_option_id as $key => $value) {
                if ($value != 0) {
                    ...
                    $option_value      = esc_sql($fetch_question_id->option_value);
                    $wpdb->query("INSERT INTO $dsp_partner_profile_question_details_table SET user_id = $current_user->ID,profile_question_id = '$key',profile_question_option_id='" . $value . "',option_value='$option_value'");
                } // End  if($value!=0) {
                    ...
                    foreach ($question_option_id2 as $k => $values) {
                        if ($values != "") {
                            foreach ($values as $key => $value) {
                              ...
                              $optionValue = esc_sql($optionValue);
                              $wpdb->query("INSERT INTO $dsp_partner_profile_question_details_table SET user_id = $current_user->ID,profile_question_id ='$k' ,profile_question_option_id='$value',option_value='" . esc_sql($optionValue) . "'");
                            }
                        } // End if($value!="")
                    } // End foreach($question_option_id1 as $key=>$value)
                }
                ...
                if (($extension != "jpg") && ($extension != "jpeg") && ($extension != "png") && ($extension != "gif")) {
                    ...
                    if (!$copied) {
                        ...
                        if ($check_approve_photos_status->setting_status == 'Y') {  // if photo approve status is Y then photos Automatically Approved.
                            ...
                            $count_rows = $wpdb->get_var("SELECT COUNT(*) FROM $dsp_members_partner_photos_table WHERE user_id=$current_user->ID");
                            if ($count_rows > 0) {
                                $wpdb->update($dsp_members_partner_photos_table,array( 'picture' => $new_name,'status_id'=>0), array('user_id' => $current_user->ID));
                                $count_rowsin_tmp = $wpdb->get_var("SELECT COUNT(*) FROM $dsp_tmp_members_partner_photos_table WHERE t_user_id=$current_user->ID");
                                if ($count_rowsin_tmp > 0) {
                                    $wpdb->query("UPDATE $dsp_tmp_members_partner_photos_table SET t_picture='$new_name',t_status_id=0 WHERE t_user_id=$current_user->ID");
                                } else {
                                    $wpdb->query("INSERT INTO $dsp_tmp_members_partner_photos_table SET t_user_id=$current_user->ID,t_picture='$new_name',t_status_id=0");
                                } //  if($count_rowsin_tmp>0){
                            } else {
                            ...
                            $approval_message = __('Pictures you uploaded will be approved within 24 hours.', 'wpdating');
...
if (isset($_GET['msg']) && $_GET['msg'] == 'ed') {
    ?>
    <div class="thanks">
        <p align="center" class="error"><?php echo __('Please update Profile Before Add to friend.', 'wpdating') ?></p>
    </div>
    <?php
}
if (isset($required_Error1) && $required_Error1 != '') {
    ...
    <div class="thanks">
        <p align="center" class="error"><?php
            ...
            echo __('Please enter ', 'wpdating') . "&nbsp;" . $req_error . "&nbsp;" . __(' values', 'wpdating');
            ?></p>
    </div>
    <?php
}
if (isset($required_Error2) && $required_Error2 != '') {
    ...
    <div class="thanks">
        <p align="center" class="error"><?php
            ...
            echo __('Please Select ', 'wpdating') . "&nbsp;" . $req_error2 . "&nbsp;" . __(' values', 'wpdating');
            ?></p>
    </div>
    <?php
}
if ($check_approve_profile_status->setting_status == 'Y') {
    if (isset($profile_updated) && $profile_updated == true) {
        ?>
        <div class="thanks">
            <p align="center" class="error"><?php echo __('Your profile has been updated!', 'wpdating') ?></p>
        </div>
        <?php
    }
} else {
...
<form name="frm_u_profile" class="dspdp-form-horizontal" action="" method="post" enctype="multipart/form-data" onsubmit="return dsp_profile_validation();
    ...
    <div style="" class="box-border">
        <div class="box-pedding">
            <div class="heading-submenu dsp-none"><?php echo __('General', 'wpdating') ?></div>
            <div class="dsp-box-container dsp-form-container dsp-space">

                <ul class="edit-profile">
                    <li class=" dsp-form-group dspdp-form-group"><span class="dsp-sm-3 dsp-control-label dspdp-col-sm-3 dspdp-control-label"><?php echo __('I am:', 'wpdating') ?></span>
                        <?php
    ...
    <div class="box-border profile-edit-page magn-top-15">
        <div class="box-pedding dsp-form-container form-horizontal">
            <div class="heading-submenu dsp-none"><?php echo __('Profile Questions', 'wpdating') ?></div>
            <div class="dsp-box-container">
                <div class="heading dsp-block dsp-space" style="display:none">
                    <h3><?php echo __('Profile Questions', 'wpdating') ?></h3>
                </div>
                <?php


                $exist_profile_options_details = $wpdb->get_results("SELECT * FROM $dsp_partner_profile_question_details_table WHERE user_id = '$current_user->ID'");
                ...
                <ul>
                    <?php
                    do_action('dsp_display_question_by_order',$update_exit_option, 1);
                    ...
                    <li class="dsp-row dsp-form-group dspdp-form-group">
                        <span class="dsp-sm-3 dsp-control-label dspdp-control-label dspdp-col-sm-3 "><?php echo __('About Me', 'wpdating') ?>:</span>
                    ...
                    ?>
                    <li class="dsp-row dsp-form-group dspdp-form-group">
                        <span class="dsp-sm-3 dsp-control-label  dspdp-control-label dspdp-col-sm-3 "><?php echo __('My Interests', 'wpdating'); ?>:</span>
                        <span class="dsp-sm-6 dspdp-col-sm-9">
            ...
            <div class="dsp-border dsp-space margin-btm-3">
                ...
                <ul class="image-edit-profile dsp-form-container dspdp-row row">
                    ...
                    <li class="dspdp-col-sm-6 dsp-sm-9 dspdp-xs-form-group">
                        ...
                        <span>   <input name="private" type="checkbox" value="Y" <?php $makePirvateStatus = isset($exist_profile_details->my_interest) ? $exist_profile_details->my_interest : '';if ( $makePirvateStatus == 'Y') { ?> checked="checked"  <?php } ?>/><?php echo __('Make Private', 'wpdating'); ?>  </span>
                    </li>
                    <li class="dspdp-col-sm-4 dsp-none"><input type="hidden" name="mode"  value="update"/>
                        <input type="submit" name="submit1" class="dsp_submit_button dspdp-btn dspdp-btn-default" value="<?php _e(__('Update', 'wpdating')); ?>" />
                    </li>
                </ul>
            </div>
