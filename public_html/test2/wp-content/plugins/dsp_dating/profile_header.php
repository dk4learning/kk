...
/////////////////////////////////////---------------------------////////////////////////////
        ...
        var meetMeFn = function() {
            ...
            if (submitted != 1) {
                if (dsp("#dsp_meet_me_box_widget").length > 0){
                    dsp("#dsp_meet_me_box_widget").fadeOut();
                }else{
            ...
            dsp.ajax({
                ...
                success : function(html) {
                    if (submitted == 1) {
                        ...
                        if (dsp("#dsp_meet_me_box_widget").length > 0){
                            dsp("#dsp_meet_me_box_widget").load("<?php echo $pluginpath . "dsp_meet_me_box.php?user_id=$user_id"; ?>");
                            dsp("#dsp_meet_me_box_widget").fadeIn();
                        }else{
        ...
        url: "<?php echo $chat_popup ?>",
                ...
                success: function(html) {
                    ...
                    {    
                        ...
                        dsp('#colorbox').addClass('cboxOneToOneChatcolorbox');
                        dsp('#cboxWrapper').addClass('cboxOneToOneChatWrapper');
...
$firstname = get_user_meta($member_id,'first_name');
    ...
    if ($wptouch_plugin_obj->applemobile || $wptouch_plugin_obj->desired_view == 'mobile') { // user is acceccing site from mobile redirect him in the mobile folder
        ...
        ?><div id="dsp_plugin" style="width:100%;" ><?php
                if (is_user_logged_in()) {  // CHECK MEMBER LOGIN 
                    ...
                    DISPLAY MEMBER NAME 
                    */
                    include_once(WP_DSP_ABSPATH . "members/loggedin/member_dsp_header.php");
                } else {
                        include_once(WP_DSP_ABSPATH . "members/withoutloggedin/guest_dsp_header.php");
                }
            ?></div><?php
        }
