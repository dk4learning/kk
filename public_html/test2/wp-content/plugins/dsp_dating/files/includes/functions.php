 ...
 *
 * @param array $user_details register user details
 * @param Integer $user_id User id
 */
if ( ! function_exists( 'dsp_add_user_details_in_meta_table' ) ) {
    function dsp_add_user_details_in_meta_table( $user_details, $user_id ) {
...
/**
 *  This function is used to change password for reset password feature
 *
 */
if ( ! function_exists('dsp_change_password')) {
    function dsp_change_password()
    {
        if ( ! wp_verify_nonce($_POST['ajax_nonce'], 'change-password-ajax-nonce')) {
            die(json_encode(array('output' => 'fail')));
        }
        global $wpdb;
        wp_set_password($_POST['password'], $_POST['user_id']);
        if(dsp_issetGivenEmailSetting($_POST['user_id'],'reset_password')) {
            $dsp_email_templates_table = $wpdb->prefix . DSP_EMAIL_TEMPLATES_TABLE;

            $email_template   = $wpdb->get_row("SELECT * FROM {$dsp_email_templates_table} WHERE mail_template_id='23'");
            $receiver_details = $wpdb->get_row("SELECT * FROM {$wpdb->users} WHERE ID='{$_POST['user_id']}'");

            $receiver_name          = $receiver_details->display_name;
            $receiver_email_address = $receiver_details->user_email;
            ...
            $wpdating_email  = Wpdating_email_template::get_instance();
            $wpdating_email->send_mail( $receiver_email_address, $email_subject, $email_message );

            die(json_encode(array('output' => 'success')));
        } else {
            die(json_encode(array('output' => 'fail')));
        }
    }
...
add_action( 'wp_ajax_dsp_change_password', 'dsp_change_password' );
add_action( 'wp_ajax_nopriv_dsp_change_password', 'dsp_change_password' );
