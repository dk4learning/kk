...
if (is_plugin_active('wpdating-api/wpdating-api.php')) {
    $dsp_mem_google_product_id = isset($_REQUEST['txtmembership_google_product_id']) ? $_REQUEST['txtmembership_google_product_id'] : '';
    $dsp_mem_apple_product_id  = isset($_REQUEST['txtmembership_apple_product_id']) ? $_REQUEST['txtmembership_apple_product_id'] : '';
}
...
function square_crop($src_image, $dest_image, $thumb_size = 64, $jpg_quality = 90)
{
    ...
    if (imagecopyresampled($canvas, $image_data, 0, 0, $x_offset, $y_offset, $thumb_size, $thumb_size, $square_size,
        $square_size)) {
...
if (isset($_POST['submit'])) {
    ...
    switch ($dsp_action) {
        case 'add':
            if ( ! empty($dsp_mem_name)) {
                ...
                if ($dsp_mem_image) {
                    ...
                    if (($extension != "jpg") && ($extension != "jpeg") && ($extension != "png") && ($extension != "gif")) {
                        ...
                        <div class="error">
                            <p>
                                <strong><?php echo __('Unknown extension for image!', 'wpdating'); ?></strong>
                            </p>
                        </div>
                        <?php
                        $errors = 1;
                        ...
                        if ( ! $copied) {
                            ...
                            <div class="error">
                                <p>
                                    <strong><?php echo __('Copy unsuccessfull!', 'wpdating'); ?></strong>
                                </p>
                            </div>
                            <?php
                            $errors = 1;
                            ...
                            if (is_plugin_active('wpdating-api/wpdating-api.php')) {
                                $wpdb->query("INSERT INTO $dsp_membership_table (`membership_id`, `name`, `google_product_id`, `apple_product_id`, `price`, `no_of_days`, `display_status`, `active_status`, `date_added`, `premium_access_feature`, `description`, `image`, `free_plan`, `stripe_recurring_plan_id`) VALUES ('','$dsp_mem_name','$dsp_mem_google_product_id', '$dsp_mem_apple_product_id', '$dsp_mem_price', '$dsp_mem_days', '$dsp_mem_status','$dsp_mem_active_status', '$dsp_mem_added_date', '$premium_access_feature_value','$dsp_mem_desc','t_" . $new_name . "', $dsp_mem_free_plan, '$dsp_stripe_recurring_plan_id')");
                            } else {
            ...
            <div class="updated">
                <p>
                    <strong><?php echo __('Membership updated!!!!', 'wpdating'); ?></strong>
                </p>
            </div>
            <?php
            break;
        ...
        case 'update':
            if ( ! empty($dsp_mem_name) && isset($dsp_mem_price) && $dsp_mem_price != "" && ! empty($dsp_mem_days)) {
                ...
                if (is_plugin_active('wpdating-api/wpdating-api.php')) {
                    $wpdb->update($dsp_membership_table, array(
                        'name'                     => $dsp_mem_name,
                        'google_product_id'        => $dsp_mem_google_product_id,
                        'apple_product_id'         => $dsp_mem_apple_product_id,
                        'price'                    => $dsp_mem_price,
            ...
            <div class="updated">
                <p>
                    <?php echo __('Membership updated!!!!', 'wpdating'); ?>
                </p>
            </div>
            <?php
            break;
...
<div id="general" class="postbox">
    ...
    <h3 class="hndle"><span><?php echo __('Memberships', 'wpdating'); ?></span></h3>
        ...
        <div class="inside">
            <table class="form-table" style="width:70%">
                ...
                <tr>
                    ...
                    <th><?php echo __('Display Status', 'wpdating'); ?></th>
                </tr>
                ...
                foreach ($myrows as $memberships) { ?>
                    <tr>
                        ...
                        if (empty($check_plan_is_active) || $check_plan_is_active == null) {
            ...
            <p class="dsp_note">
                <?php echo __('Please Note: If a membership plan is being used you cannot delete that Membership Plan.', 'wpdating'); ?>
            </p>
            <p class="dsp_note">
                <?php echo __('Due to restrictions put on recurring plan, recurring payment gateway (such as Paypal Recurring - Subscriptions) can only be activated where the number of days satisfies the following conditions.', 'wpdating'); ?>
            </p>
            <p class="dsp_note">
                <?php echo '1.' . __('No of days is less than or equal to 90 with range (1 to 90)', 'wpdating') . '<br>' . '2.' . __('No of days is multiple of 7 with range (7 to 364).', 'wpdating') . '<br>' . '3.' . __('No of days is multiple of 30 with range (30 to 720).', 'wpdating') . '<br>' . '4.' . __('No of days is multiple of 365 with range (365 to 1825).', 'wpdating');
                ?>
            </p>
        </div>
...
if (isset($_GET['Action']) && $_GET['Action'] == 'update') {
    ...
    if (is_plugin_active('wpdating-api/wpdating-api.php')) {
        $google_product_id = $dsp_updates->google_product_id;
        $apple_product_id  = $dsp_updates->apple_product_id;
    }
...
<div id="general" class="postbox">
    ...
    <h3 class="hndle"><span><?php echo __('Custom Memberships', 'wpdating'); ?></span></h3>
    ...
    <form name="membershipfrm" method="post" enctype="multipart/form-data">
        <div class="inside">
            <table class="form-table">
                ...
                <tr>
                    <th>
                        <label><?php _e(__('Name', 'wpdating')); ?></label>
                    </th>
                    <td>
                ...
                <?php if (is_plugin_active('wpdating-api/wpdating-api.php')) { ?>
                    <tr>
                        <th>
                            <label>Google Product ID</label>
                        </th>
                        <td>
                            <input type="text" name="txtmembership_google_product_id"
                                   value="<?php echo $google_product_id; ?>"
                                   class="regular-text"/>
                        </td>
                    </tr>
                    ...
                    <tr>
                        ...
                        <td>
                            <input type="text" name="txtmembership_apple_product_id"
                                   value="<?php echo $apple_product_id; ?>"
                                   class="regular-text"/>
                        </td>
                    </tr>
                <?php } ?>
                <tr>
                    <th>
                        <label><?php _e(__('Price', 'wpdating')); ?></label>
                    </th>
                    <td>
                ...
                <tr>
                    ...
                    <td>
                        ...
                        <?php _e(__('Make this membership plan free', 'wpdating')); ?><br>
                        <br>
                    </td>
                </tr>
                ...
                <tr>
                    <th>
                        <label><?php _e(__('Number of Days', 'wpdating')); ?></label>
                    </th>
                    <td>
                ...
                <tr>
                    <th>
                        <label><?php _e(__('Description', 'wpdating')); ?></label>
                    </th>
                    <td>
                ...
                <tr>
                    <th>
                        <label><?php _e(__('Upload Photo:', 'wpdating')); ?></label>
                    </th>
                    <td>
                        <?php
                        if (isset($_GET['Action']) && $_GET['Action'] == 'update') {
                        ...
                        <tr>
                            ...
                            <td>
                                <input name="stripe_recuring_plan_id" type="text" id="stripe_recuring_plan_id"
                                       class="regular-text"
                                       value="<?php echo isset($membership_stripe_recurring_plan_id) ? $membership_stripe_recurring_plan_id : '' ?>">
                            </td>
                        </tr>
                        <?php
                    }
...
<script>
    function Checkform() {
        ...
        if (document.membershipfrm.txtmembership_product_id.value == "") {
            alert('please choose membership product id.');
            document.membershipfrm.txtmembership_product_id.focus();
            return false;
        }
