...
$help_tab = isset($_REQUEST['help_tab']) ? $_REQUEST['help_tab'] : '';
$stealth_mode_option = isset($_REQUEST['stealth_tab']) ? $_REQUEST['stealth_tab'] : '';
...
// Min Age
$dsp_min_age = isset($_REQUEST['dsp_min_age']) ? $_REQUEST['dsp_min_age'] : '';

//Max Age
$dsp_max_age = isset($_REQUEST['dsp_max_age']) ? $_REQUEST['dsp_max_age'] : '';

if($random_online_member_numbers > 10){
    $_SESSION['errors']['exceeds_random_online_nos'] = __('Please enter less than 10 for random online members', 'wpdating');
}else{
    ...
    $wpdb->query("UPDATE $dsp_general_settings_table SET setting_status = '$help_tab' WHERE setting_name = 'help_tab'");
    $wpdb->query("UPDATE $dsp_general_settings_table SET setting_status = '$stealth_mode_option' WHERE setting_name = 'stealth_mode'");
    ...
    if ($registerurltxt != '') {
        ...
        ## Min age
        $sql = "SELECT setting_value FROM $dsp_general_settings_table WHERE setting_name = 'min_age'";
        if($wpdb->get_row($sql)) {
            $wpdb->query("UPDATE $dsp_general_settings_table SET setting_value ='$dsp_min_age' WHERE setting_name = 'min_age'");
        } else {
            $wpdb->query("INSERT INTO $dsp_general_settings_table SET setting_status='Y', setting_value ='$dsp_min_age', setting_name = 'min_age'");
        }

        ## Max age
        $sql = "SELECT setting_value FROM $dsp_general_settings_table WHERE setting_name = 'max_age'";
        if($wpdb->get_row($sql)) {
            $wpdb->query("UPDATE $dsp_general_settings_table SET setting_value ='$dsp_max_age' WHERE setting_name = 'max_age'");
        } else {
            $wpdb->query("INSERT INTO $dsp_general_settings_table SET setting_status='Y', setting_value ='$dsp_max_age', setting_name = 'max_age'");
        }

        $_SESSION['message'] = __('Settings changes are saved.', 'wpdating');
    }
}    
?>
