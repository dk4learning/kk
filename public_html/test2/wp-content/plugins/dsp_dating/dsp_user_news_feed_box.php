...
<ul class="news-feed-page">
    ...
    foreach ($news_feed as $row) {
        ...
        if ($row->feed_type == "status")
            $news_feeds[$row->feed_id]['feed_text'] = '<span class="status_wrap"><span>'. $username .'</span>' . ' ' . __('just added new status update.', 'wpdating').'</span>';
        else if ($row->feed_type == "login")
            $news_feeds[$row->feed_id]['feed_text'] = '<span class="status_wrap status_login"><span>'. $username .'</span>' . ' ' . __('just logged in.', 'wpdating').'</span>';
        else if ($row->feed_type == "logout")
            $news_feeds[$row->feed_id]['feed_text'] = '<span class="status_wrap status_logout"><span>'. $username .'</span>' . ' ' . __('just logged off.', 'wpdating').'</span>';
        else if ($row->feed_type == "video")
            $news_feeds[$row->feed_id]['feed_text'] = '<span class="status_wrap"><span>'. $username .'</span>' . ' ' . __('just added new video.', 'wpdating').'</span>';
        else if ($row->feed_type == "audio")
            $news_feeds[$row->feed_id]['feed_text'] = '<span class="status_wrap"><span>'. $username .'</span>' . ' ' . __('just added new audio.', 'wpdating').'</span>';
        else if ($row->feed_type == "gallery_photo")
            $news_feeds[$row->feed_id]['feed_text'] = '<span class="status_wrap"><span>'. $username .'</span>'. ' ' . __('just added new photo.', 'wpdating').'</span>';
        else if ($row->feed_type == "profile_photo")
            $news_feeds[$row->feed_id]['feed_text'] = '<span class="status_wrap"><span>'. $username .'</span>' . ' ' . __('just added new profile photo.', 'wpdating').'</span>';
        ...
        $news_feeds[$row->feed_id]['stat'] = $row->feed_type;
    }
    ...
    foreach ($news_feeds as $news_row) {
        ?>
        <li class="<?php if( $news_row['stat'] == 'login'|| $news_row['stat'] == 'logout' ){ echo 'hide_login_logout';} ?>">
        <?php 
        echo '<div class="dspdp-bordered-item"><a href="';
    ...
    echo '<li>' . $pagination . '</li>';
        ...
        echo '<li>'.__('No record found for your search criteria.', 'wpdating').'</li>';
    endif; ?>
</ul>