...
jQuery(document).ready(function($) {
	...
	$('.pkg_media_add').on('click', function(){
       ...
       var imagepath = scriptjsdata.image_path+'/images/upload-image.png';
		$ul.append('<li><div class="banner-admin-left"><img class="media_image" src="'+ imagepath +'"></div><div class="banner-admin-right"><label class="banner-admin-right-label">Banner Image Path</label><input type="text" name ="bpslider_imageurl[]" value="" class="media_imageurl bpslider_imageurl" /><label class="banner-admin-right-label">Banner Heading</label><input type="text" name ="bpslider_link[]" value="" class="bpslider_link" /><a href="#" class="pkg_option_remove"><span>x</span></a><label class="banner-admin-right-label">Banner Text</label><input type="text" name="bpslider_text[]" value="" class="bpslider_text" /><label class="banner-admin-right-label">Banner Button Link</label><input type="text" name="bpslider_button[]"  value="" class="bpslider_button" /><label class="banner-admin-right-label">Banner Button Text</label><input type="text" name="bpslider_buttontext[]"  value="" class="bpslider_buttontext" /></div></li>');
	...
	$('.tab_content_gallery').on('click', '.media_image',  function() {	// when image is clicked
		// Get the Text element.
		//console.log($( this ).parent().next('.banner-admin-right'));
		var text = $( this ).parent().next('.banner-admin-right').find('.media_imageurl');
		//console.log($( this ).parent().next('.banner-admin-right').find('.media_imageurl'));
		...
		window.send_to_editor = function( html ) {
			...
			text.attr( 'value', src ).change(function(){
				// bind the value of this field to preview field
				$( this ).parent().parent().find('.media_image').attr({'src':this.value}).addClass('no_background');
			}).change();
			tb_remove(); // Then close the popup window
		}
		return false;
