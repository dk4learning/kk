...
get_header(); ?>
    ...
    <h1><?php _e('Archive for: ', 'wp-dating'); ?><strong><?php single_cat_title(); ?></strong></h1>
  </section>
    ...
    <div class="content">
      ...
      <article id="post-<?php the_ID(); ?>" <?php post_class( 'group' ); ?> role="article">
          ...
          <?php if(is_user_logged_in())  edit_post_link( __( 'edit', 'wp-dating' ) ); ?>
				</header>
        <?php if(has_post_thumbnail()) the_post_thumbnail('image21'); ?>
				<div class="post-content group">
          <?php the_excerpt(); ?>
          <a href='<?php the_permalink(); ?>'><?php _e('READ MORE <span>&raquo;</span>', 'wp-dating'); ?></a>
        </div>
        <footer>
