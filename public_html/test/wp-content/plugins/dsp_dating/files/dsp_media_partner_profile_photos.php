...
$dsp_members_partner_photos = $wpdb->prefix . DSP_MEMBERS_PARTNER_PHOTOS_TABLE;
...
if ($Action == "Delete" && !empty($member_ids)) {
    $fetch_member_picture = $wpdb->get_row("SELECT * FROM $dsp_members_partner_photos Where user_id='$member_ids'");
    ...
    if ($member_ids != "") {
        ...
        $wpdb->query("DELETE from $dsp_members_partner_photos where user_id='$member_ids'");
    } // if($member_ids!="")
} // if(Action=="Delete")
// ###########################  delete approve image ########################################
// ###########################  delete image ########################################
if ($Action == "Del" && !empty($member_ids)) {
    $fetch_member_pic = $wpdb->get_row("SELECT * FROM $dsp_tmp_members_partner_photos_table Where t_user_id='$member_ids'");
    ...
    if ($member_ids != "") {
        ...
        $wpdb->query("DELETE from $dsp_tmp_members_partner_photos_table where t_user_id='$member_ids'");
    } // if($member_ids!="")
} // if(Action=="Del")
// ###########################  Approve Image ########################################
...
if ($Action == "approve" && !empty($member_ids)) {
    ...
    $exist_photo = $wpdb->get_var("SELECT COUNT(*) as Num FROM $dsp_members_partner_photos where user_id='$user_id'");
    if ($exist_photo > 0) {
        $wpdb->query("UPDATE $dsp_members_partner_photos SET picture='$picture',status_id='1' WHERE user_id='$member_ids'");
    } else {
        $wpdb->query("INSERT INTO $dsp_members_partner_photos SET user_id='$member_ids',picture='$picture',status_id='1'");
    }
...
if ($Action == "reject" && !empty($member_ids)) {
    ...
    $fetch_member_pic = $wpdb->get_row("SELECT * FROM $dsp_tmp_members_partner_photos_table Where t_user_id='$member_ids'");
    $wpdb->query("UPDATE $dsp_tmp_members_partner_photos_table SET t_status_id='2' WHERE t_user_id='$member_ids'");
...
if ($list_status == 1) {
    ...
    $total_results1 = $wpdb->get_var("SELECT COUNT(*) as Num FROM $dsp_members_partner_photos photos, $dsp_user_profiles_table profile WHERE photos.user_id=profile.user_id AND photos.status_id = '$list_status'");
} else {
    $total_results1 = $wpdb->get_var("SELECT COUNT(*) as Num FROM $dsp_tmp_members_partner_photos_table WHERE t_status_id='$list_status'");
}
                            ...
                            if (isset($_POST['search'])) {
                                ...
                                if ($username != '') {
                                    ...
                                    foreach ($search_username as $username) {
                                        ...
                                        $myrows = $wpdb->get_results("SELECT photos.picture,photos.user_id FROM $dsp_members_partner_photos photos, $dsp_user_profiles_table profile WHERE photos.user_id=profile.user_id AND photos.status_id = '$list_status' AND photos.user_id='$user_id' LIMIT $start, $limit");
                                    }
                                } else {
                                    $myrows = $wpdb->get_results("SELECT photos.picture,photos.user_id FROM $dsp_members_partner_photos photos, $dsp_user_profiles_table profile WHERE photos.user_id=profile.user_id AND photos.status_id = '$list_status' LIMIT $start, $limit");
                                }
                            }   //if(isset($_POST['search']))
                            else
                                $myrows = $wpdb->get_results("SELECT photos.picture,photos.user_id FROM $dsp_members_partner_photos photos, $dsp_user_profiles_table profile WHERE photos.user_id=profile.user_id AND photos.status_id = '$list_status' LIMIT $start, $limit");
                        } else {
                            $myrows = $wpdb->get_results("SELECT t_picture,t_photo_id,t_user_id FROM $dsp_tmp_members_partner_photos_table WHERE t_status_id ='$list_status' LIMIT $start, $limit");
                        }
                        $i = 0;
                            ...
                            if ($list_status == 0) {
                                ...
                                <td align="left" class="thumbnails-bg-pix">
                                    <table cellpadding="0" border="0">
                                        ...
                                        <tr>
                                            <td><span onclick="approve_images('<?php echo $pic_mem_id ?>')" class="span_pointer" style="font-size:12px;"><?php echo __('Approve', 'wpdating') ?></span>|</td>
                                            <td><span onclick="reject_images('<?php echo $pic_mem_id ?>')" class="span_pointer" style="font-size:12px;"><?php echo __('Reject', 'wpdating') ?></span>|</td>
                                            <td><span onclick="delete_images('<?php echo $pic_mem_id ?>')" class="span_pointer" style="font-size:12px;"><?php echo __('Delete', 'wpdating'); ?></span></td></tr>
                                    </table>
                                </td>
                                <?php
                                    ...
                                    <table cellpadding="0" border="0">
                                        ...
                                        <tr>
                                            <td><span onclick="delete_approve_images('<?php echo $pic_mem_id ?>')" class="span_pointer" style="font-size:12px;margin-left:22px;text-decoration:none;">
                                                    <input class="button" name="<?php echo __('Delete', 'wpdating'); ?>" type="button" value="<?php echo __('Delete', 'wpdating'); ?>" /></span></td>
                                        </tr>
                                    </table>
                                </td>
                                <?php
