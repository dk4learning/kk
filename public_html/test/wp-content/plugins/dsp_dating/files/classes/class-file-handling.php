	...
	public function uploadFile() {
        ...
        if ($fileName) {
            ...
            if (!in_array($extension,$this->_validExtension)) {
                $this->errors[] = __('Unknown extension for image!', 'wpdating') . '<br>';
                $invalidEntry = 1;
                $errors = 1;
                $fileName = "";
                return false;
                ...
                if ($sizekb > MAX_SIZE * 1024) {
                    $this->errors[] = __('Image is too large please reduce flag image below ', 'wpdating') . MAX_SIZE . DSP_USER_KB . '<br>';
                    $errors = 1;
                    $fileName = "";
                    $invalidEntry = 1;
                    return false;
                ...
                if ($height < MIN_HEIGHT || $width < MIN_WIDTH) {
                    $this->errors[] = __('Minimum ', 'wpdating') . MIN_HEIGHT . 'x' . MIN_WIDTH . __(' Dimension is required for image.', 'wpdating') . '<br>';
                    $errors = 1;
                    $fileName = "";
                    $invalidEntry = 1;
                    return false;
