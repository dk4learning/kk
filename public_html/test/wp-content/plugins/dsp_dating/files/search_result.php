...
// ----------------------------------------------- Start Paging code------------------------------------------------------ //
    ...
    if ($fetch_record_by_save_search != "") {
        ...
        if (isset($_REQUEST['Online_only'])) {
            ...
            $Online_only = 'N';
        $profile_question_option_id1 = empty($page) ? esc_sql($_REQUEST['profile_question_option_id']) : get('profile_question_option_id');
...
//$strQuery = "SELECT DISTINCT (fb.user_id) FROM $dsp_user_profiles fb WHERE fb.stealth_mode != 'Y' AND ";
    ...
    if ($Online_only == "Y") {
        ...
        $strQuery .= "INNER JOIN $tbl_name onl ON fb.user_id = onl.user_id WHERE  ";
    ...
    if (($cmbCountry <= 0) && ($profile_question_option_id > 0) && ($Pictues_only == 'P') && ($cmbState <= 0) && ($cmbCity <= 0)) {
        $strQuery = substr($strQuery, 0, strlen($strQuery) - 7);
        $strQuery .= " INNER JOIN $dsp_question_details o ON fb.user_id = o.user_id WHERE  (";
        foreach ($profile_question_option_id1 as $key => $value) {
            $strQuery .= " o.profile_question_option_id = $value AND";
        }
    ...
    <div id="search_width" class="box-border">
        <div class="box-pedding dspdp-row">
            ...
            foreach ($search_members as $member1) {
                ...
                ?>
               <div class="dspdp-col-sm-4 dsp-sm-4"> <div class="box-search-result image-container">
                                ...
                                <?php if ($s_make_private == 'Y') { ?>
                                    ...
                                    <?php if ($current_user->ID != $s_user_id) { ?>
                                        ...
                                        <?php } else {
                                            ?>
                                            <a href="<?php echo $root_link . get_username($s_user_id) . "/"; ?>" >
                                                <img src="<?php echo display_members_photo_thumb($s_user_id, $imagepath); ?>"   alt="<?php echo get_username($s_user_id); ?>"   border="0" class="img-big"/></a>
                                            <?php
                                        }
                                    } else {
                                        ?>
                                        <a href="<?php echo $root_link . get_username($s_user_id) . "/"; ?>">
                                            <img src="<?php echo display_members_photo_thumb($s_user_id, $imagepath); ?>" alt="<?php echo get_username($s_user_id); ?>"  border="0" class="img-big" />
                                        </a>
                                    <?php } ?>
                                <?php } else { ?>
                    ...
                    <?php if(isset($check_distance_mode->setting_status) && $check_distance_mode->setting_status == 'Y'  && $search_type == "distance_search"): ?>
                        <div class="user-details dspdp-spacer dspdp-user-details">
