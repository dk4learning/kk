...
<div role="banner" class="ui-header ui-bar-a" data-role="header">
    ...
    <h1 aria-level="1" role="heading" class="ui-title"><?php echo __('Compose', 'wpdating'); ?></h1>
...
if ($mode == "sent") {
    if (trim($_REQUEST['friend_id']) == 0) {
        ...
        $to_Error = __('You forgot to select your Friend.', 'wpdating');
    ...
    if (trim($_REQUEST['txtSubject']) === '') {
        $subjectError = __('You forgot to enter Subject.', 'wpdating');
        $hasError = true;
    } else {
    ...
    if (trim($_REQUEST['txtmessage']) === '') {
        $messageError = __('You forgot to enter Message.', 'wpdating');
    ...
    if (trim($check_spam_filter->setting_status) === 'Y') {
        ...
        if (isset($spam_words) && $spam_words != "") {
            $spam_messageError = __('Could not continue as text entered contains words included in our spam filter.', 'wpdating');
            $hasError = true;
        } else {
            $message = trim($_REQUEST['txtmessage']);
        }
    ...
    if ($checked_block_member > 0) {
        $blocked_Error = __('Sorry,You are blocked by this user. You can not send a message.', 'wpdating');
        $hasError = true;
    } else {
        $friend_id = trim($_REQUEST['friend_id']);
    }
    ...
    if (!isset($hasError)) {
        ...
        if ($check_friend_notification <= 0) {
            ...
            $message_sent = __('Message Sent Sucessfully.', 'wpdating');
            ...
            $message_sent = __('Alert Member Notifications-> You can&rsquo;t send a Message to this Member.', 'wpdating');
        }
    }
}
?>
...
<div data-role="content" class="ui-content" role="main">
 ...
 if (isset($message_sent) && $message_sent != "") {
    ...
    <form name="composefrm" id="composefrm">
        ...
        <fieldset>
            ...
            <label data-role="fieldcontain" class="form-group">  
              <div class="clearfix">
              <div class="mam_reg_lf form-label"><?php echo __('To', 'wpdating'); ?></div>
                  <?php 
    ...
    <label data-role="fieldcontain" class="select-group">  
      <div class="clearfix">
          <div class="mam_reg_lf select-label"><?php echo __('To', 'wpdating'); ?></div>
                ...
                if ($chk_user_exist != 0) {
                        ...
                        <?php if (isset($to_Error) && $to_Error != '') {
                            ...
                            <div data-role="fieldcontain" class="ui-hide-label">
                             <input type="text" class="text-box spacer-bottom-sm" name="txtSubject" value="<?php if (isset($reply_Subject)){ echo $reply_Subject;}else { echo "";} ?>" placeholder="<?php echo __('Subject', 'wpdating'); ?>"/>
                         </div>
                         ...
                         <?php if (isset($subjectError) && $subjectError != '') {
                            ...
                            <?php if (isset($messageError) && $messageError != '') {
                                ...
                                <div> <span><?php echo __('Message', 'wpdating') ?>:</span></div>
                            ...
                            <div data-role="fieldcontain" >
                                ...
                                <textarea  name="txtmessage"  class="textarea-box" placeholder="<?php echo __('Message', 'wpdating') ?>"><?php if (isset($reply_message_contentmain)) echo "\n\n" . $reply_message_contentmain; ?></textarea>
                            </div>
                            <div data-role="fieldcontain">
                                ...
                                <div class="btn-blue-wrap">
                                    <input type="button" class="mam_btn btn-blue" onclick="getCompose('true')" id="sent1" name="sent1" value="<?php echo __('Send Message', 'wpdating'); ?>"></div>
