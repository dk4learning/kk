...
?>
        ...
        <li data-corners="false" data-shadow="false" class="ui-body ui-body-d ui-corner-all">
             <div class="dsp_mail_lf">
                ...
                <div style="font-size: 13px; font-weight: bold; text-align: center;padding-top: 10px;">
                    <div style="padding-bottom: 10px;">
                        ...
                        <span><?php echo __('Credits', 'wpdating'); ?></span>
                    </div>
                    <?php echo $currency_code ?><?php echo $credit_row->price_per_credit; ?>&nbsp;<?php echo __('Per Credit', 'wpdating'); ?>
                    ...
                    if ($gateway->gateway_name == 'paypal' && $gateway->status == 1) {
                        ...
                        if ($clientID != "" && $clientSecret != "" && $apiUsername != "" && $apiPassword != "" && $apiSignature != "" && $business != "" && $currency_code != "") {
                            ...
                            <div class="btn-blue-wrap">
                                <form id="frm_paypal" name="frm_paypal" >
                                    ...
                                    <input type="hidden" id="creditTitle" name="creditTitle" value="<?php echo __('Purchase Credits', 'wpdating'); ?>" />
                                    <input type="hidden" id="user_id" name="user_id" value="<?php echo $user_id ?>" />

                                    <input id="paypalBtn" onclick="getCredit()" class="mam_btn btn-blue" name="upgrade_credit" title="Upgrade / PayPal" type="button" value="<?php echo __('Paypal', 'wpdating') ?>"  style="text-decoration:none;" />
                                    <br />  
                            ...
                            ?>
                            <div class="btn-blue-wrap">
                                <input id="paypalBtn" type="button" name="paypalbtn" class="mam_btn btn-blue" onclick="gateWayDisable('<?php echo __('Admin paypal details are not set yet. Please try again later.', 'wpdating'); ?>')" value="<?php echo __('Paypal', 'wpdating'); ?>"/>
                            </div>
                            <?php
                        }
                        ...
                        <div class="btn-blue-wrap">
                            <form id="frm_auth">
                                ...
                                <input type="hidden" class="no_of_credit_to_purchase" name="no_of_credit_to_purchase" value="1" />
                                <input onclick="callUpgrade('credit_auth_setting', '0')"  id="paypalBtn" class="mam_btn btn-blue" name="upgrade" title="Upgrade / Credit Card" type="button" value="<?php echo __('Credit Card', 'wpdating') ?>" class="dsp_span_pointer" style="text-decoration:none; margin-top:5px;" />
                            </form>
                        ...
                        <div class="btn-blue-wrap">
                            <form id="frm_pro" name="frm_pro" >
                                <input onclick="callUpgrade('credit_pro_settings', '0')" id="paypalBtn" class="mam_btn btn-blue" name="upgrade" title="Upgrade / PayPal Pro" type="button" value="<?php echo __('Paypal Pro', 'wpdating') ?>" />
                                <input type="hidden" name="credit_amount" class="credit_amount" value="<?php echo $credit_row->price_per_credit; ?>" /> 
                                <input type="hidden"  name="user_id" value="<?php echo $user_id; ?>" />
                         ...
                         *  if($recurringPaymentSatatus)
                          ...
                          <?php echo __('You are already a ', 'wpdating'). $name.' '.__('plan member.', 'wpdating');?>
                          </div>
                          <?php }
                          else
                          { 				?>
                          ...
                          <!--<input type="submit" value="<?php echo __('Upgrade/Paypal Advance', 'wpdating')?>" name="btn_advance" />-->
                        ...
                        /* in dating app we will not show  iDEAL payment option
                          ...
                          <input id="paypalBtn" name="upgrade" title="Upgrade / iDEAL" type="submit" value="<?php echo __('Upgrade / iDEAL', 'wpdating')?>" class="dsp_span_pointer" style="text-decoration:none;" />
                          <input type="hidden" name="credit_amount" class="credit_amount" value="<?php echo $credit_row->price_per_credit;?>" />
            ...
            <div style="text-align: left; word-wrap: break-word;padding-top: 10px;float: left;width: 100%;">
                <?php echo __('Purchase Credits', 'wpdating'); ?>
            </div>
