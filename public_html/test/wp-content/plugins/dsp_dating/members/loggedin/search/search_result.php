...
// ----------------------------------------------- Start Paging code------------------------------------------------------ //
    ...
    if ($fetch_record_by_save_search != "") {
        ...
        $age_to = isset($_REQUEST['age_to']) ? esc_sql(sanitizeData($_REQUEST['age_to'], 'int')) : get('age_to'); 
        $age_from = isset($_REQUEST['age_from']) ? esc_sql(sanitizeData($_REQUEST['age_from'], 'int')) : get('age_from');
        ...
        $constantValues = $unit = isset($_REQUEST['unit']) ? esc_sql(sanitizeData(trim($_REQUEST['unit']), 'xss_clean')) : get('unit');
        $distance = isset($_REQUEST['distance']) ? esc_sql(sanitizeData(sanitizeData(trim($_REQUEST['distance']), 'int'), 'xss_clean')) : get('distance');
        $latlngSet = !empty($lat) && !empty($lng) ? true : false;
        $countryName = urldecode($countryName);
        if(empty($constantValues) || $constantValues == 0){
            $constantValues = 3959;
        }
        $types = ($constantValues == 3959) ? __('Miles', 'wpdating') : __('Km', 'wpdating');
        /* ------------------------ */
        $kontry = '';
        ...
        if (strlen($countryName) > 1) {
            $cmbCountryid = (int) $countryName;
            $get_Country = $wpdb->get_row("SELECT * FROM $dsp_country_table WHERE country_id = '" . $countryName . "'");
        ...
        if ($stateName != "Select" && strlen($stateName) > 1) { 
            $get_State = $wpdb->get_row("SELECT * FROM $dsp_state_table WHERE state_id = '" .$stateName . "'");
        ...
        if ($cityName != "Select" && strlen($cityName) > 1) {
            if ($cmbStateid == 0) {
                $get_City = $wpdb->get_row("SELECT * FROM $dsp_city_table WHERE city_id = '" . $cityName . "' and country_id=" . $cmbCountryid);
            } else {
                $get_City = $wpdb->get_row("SELECT * FROM $dsp_city_table WHERE city_id = '" . $cityName . "' and state_id=" . $cmbStateid);
            }
        ...
        if (isset($_REQUEST['Online_only'])) {
            ...
            $Online_only = 'N';
        $profile_question_option_id1 = (empty($page) && array_key_exists('profile_question_option_id', $_REQUEST))
                                                ? esc_sql($_REQUEST['profile_question_option_id']) : get('profile_question_option_id');

        $totalQuesId = '';
        if (is_array($profile_question_option_id1)) {
...
//$strQuery = "SELECT DISTINCT (fb.user_id) FROM $dsp_user_profiles fb WHERE fb.stealth_mode != 'Y' AND ";
    ...
    if ($Online_only == "Y") {
        ...
        $strQuery .= "INNER JOIN $dsp_user_online_table onl ON fb.user_id = onl.user_id WHERE  ";
...
//-------------------------------------------
    ...
    if (($cmbCountry > 0) && ($cmbState <= 0) && ($cmbCity > 0) && ($profile_question_option_id > 0) && ($Pictues_only == 'Y' || $Pictues_only == 'N' || $Pictues_only == 'P' )) {
    ...
    if ($bolIfSearchCriteria) {
        if (isset($username) && $username != '') {
            if($cmbCountry<=0)
                $strQuery = "SELECT DISTINCT (fb.user_id) FROM $dsp_user_profiles fb, $dsp_user_table users  WHERE fb.user_id=users.ID AND users.user_login like'%$username%' AND fb.gender='".$seeking."' ORDER BY fb.user_profile_id desc  " ;
            else if($cmbCountry>0){
                $strQuery = "SELECT DISTINCT (fb.user_id) FROM $dsp_user_profiles fb, $dsp_user_table users  WHERE fb.user_id=users.ID AND fb.country_id='".$cmbCountry."' AND users.user_login like'%$username%' AND fb.gender='".$seeking."' ORDER BY fb.user_profile_id desc";
...

            {
               ...
               <div class="dspdp-col-sm-4 dsp-sm-3"> <div class="dsp-search-result-item box-search-result image-container">
                    ...
                    <?php if(   isset($check_distance_mode->setting_status) &&
                     ...
                     ?>
                        <div class="user-details dspdp-spacer dspdp-user-details">
                           <?php echo __('Distance', 'wpdating') . ':' . number_format($s_distance,2) . " {$types}"; ?>
                        </div>
                    <?php endif; ?>
                                ...
                                <div class="dsp_fav_link_border" >
                                    ...
                                    if (is_user_logged_in()) {  // CHECK MEMBER LOGIN
                                        if (isset($check_my_friends_list) && $check_my_friends_list > 0) {
                                            ?>
                                            <a href="<?php echo $root_link . "email/compose/frnd_id/" . $member_id . "/Act/send_msg/"; ?>"  title="<?php echo __('Send Message', 'wpdating'); ?>">
        ...
        <div class="box-border">
            <div class="box-pedding">
                <div class="page-not-found">
                    ...
                    <span><a href="<?php echo $root_link . "search/$searchType/"; ?>"><?php echo __('Start a new Search', 'wpdating'); ?></a></span>
                </div>
            </div>
        </div>
    <?php } ?>
